...
# 生成签名
SIGNATURE=$(generate_aws_signature_v4 "$HTTP_METHOD" "$CANONICAL_URI" "$CANONICAL_QUERYSTRING" "$CANONICAL_HEADERS" "$SIGNED_HEADERS" "$PAYLOAD_HASH" "$TIMESTAMP" "$REGION" "s3" "$AWS_ACCESS_KEY" "$AWS_SECRET_KEY")
...
log_info "Authorization: $AUTH_HEADER"
