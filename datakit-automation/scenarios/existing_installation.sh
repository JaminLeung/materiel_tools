...
# 加载安装工具模块
source "$SCENARIO_PROJECT_ROOT/install/install_utils/s3_download.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_utils/machine_specs.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_utils/package_install.sh"

# 加载安装步骤模块
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_01_host_info.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_02_status_check.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_03_resource_limit.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_04_download.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_05_install.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_06_configure.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_07_setup_cron.sh"
source "$SCENARIO_PROJECT_ROOT/install/install_steps/step_08_verify.sh"
...
handle_error() {
    ...
    # 如果失败在安装过程中，尝试回滚
    if [[ "$failed_step" == "step_05_install" ]]; then
        log_warning "尝试回滚安装..."
...
execute_existing_installation() {
    ...
    # 执行安装步骤
    # 步骤1: 获取主机信息
    INSTALLATION_STATE["current_step"]="step_01_host_info"
    log_info "=== 步骤1: 获取主机信息 ==="
    step_01_get_host_info
    
    # 步骤2: 检查安装状态
    INSTALLATION_STATE["current_step"]="step_02_status_check"
    log_info "=== 步骤2: 检查安装状态 ==="
    step_02_check_installation_status
    
    # 步骤3: 设置资源限制
    INSTALLATION_STATE["current_step"]="step_03_resource_limit"
    log_info "=== 步骤3: 设置资源限制 ==="
    ...
    # 步骤6: 配置和验证
    INSTALLATION_STATE["current_step"]="step_06_configure"
    log_info "=== 步骤6: 配置和验证 ==="
    step_06_configure_and_verify
    
    # 步骤7: 设置定时任务
    INSTALLATION_STATE["current_step"]="step_07_setup_cron"
    log_info "=== 步骤7: 设置定时任务 ==="
    step_07_setup_cron_jobs
    
    # 步骤8: 验证安装结果
    INSTALLATION_STATE["current_step"]="step_08_verify"
    log_info "=== 步骤8: 验证安装结果 ==="
